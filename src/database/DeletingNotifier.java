...
public class DeletingNotifier implements DatabaseObserver {
    @Override
    public void update(Movie deletedMovie) {
        ...
        for(User user : Database.getDatabase().getUsers()) {
            if(user.getPurchasedMovies().contains(deletedMovie)) {
                user.getNotifications().add(new Notification.Builder(deletedMovie.getName(), "DELETE").build());
